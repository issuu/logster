...
BOT_PATTERN = re.compile('.*( Ezooms/|WinHttp\.WinHttpRequest|heritrix/|Crawler|Bot|Spider|AndroidDownloadManager|URL2File/|Apache-HttpClient/|PHP/|Wget/|<\?php |(http://|\w+@)\w+(\.\w+)+)')
    ...
    def parse_line(self, line):
        ...
        if __m:
            ...
            if not is_spider and not is_img_proxy:
                if client_ip.iptype() != 'PRIVATE' and __d['srvname'] == 'BACKEND':
                    try:
